...
namespace SprykerTest\Zed\SecurityOauthKnpu\Communication\Plugin\SecurityOauthMerchantPortal;
 ...
 * @group KnpuOauthMerchantUserClientStrategyPluginTest
 * Add your own group annotations below this line
 */
class KnpuOauthMerchantUserClientStrategyPluginTest extends Unit
{
    protected const string TEST_CLIENT_NAME = 'keycloak-mp';

    protected const string TEST_STATE_PREFIX = 'kmp';

    protected SecurityOauthKnpuCommunicationTester $tester;

    public function testIsApplicableReturnsTrueWhenStateMatchesConfiguredMerchantPortalProvider(): void
    {
        // Arrange
        $providerConfig = $this->tester->buildProviderConfig(static::TEST_CLIENT_NAME, static::TEST_STATE_PREFIX, 'Login');
        $factory = $this->tester->getBusinessFactoryWithMerchantUserProviderConfigs([$providerConfig]);

        $plugin = new KnpuOauthMerchantUserClientStrategyPlugin();
        ...
        // Assert
        $this->assertTrue($isApplicable, 'Expected true when state matches a configured merchant portal provider prefix.');
    }
    ...
    public function testGetResourceOwnerReturnsSuccessfulResponseWhenTokenExchangeSucceeds(): void
    {
        ...
        $resourceOwnerMock->method('getId')->willReturn('ext-mp-user-001');
        $resourceOwnerMock->method('toArray')->willReturn(['email' => 'merchant@spryker.com']);
        ...
        $providerConfig = $this->tester->buildProviderConfig(static::TEST_CLIENT_NAME, static::TEST_STATE_PREFIX, 'Login');
        $factory = $this->tester->getBusinessFactoryWithMerchantUserProviderConfigs([$providerConfig], $clientRegistryMock);
