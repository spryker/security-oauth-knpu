...
class OauthUserResourceOwnerReader implements OauthUserResourceOwnerReaderInterface
{
    use LoggerTrait;
    ...
    protected function findProviderConfigByState(?string $state): ?OauthKnpuProviderConfigTransfer
    {
        ...
        foreach ($this->securityOauthKnpuConfig->getZedUserProviderConfigs() as $oauthKnpuProviderConfigTransfer) {
    ...
    protected function getOauthClientByConfig(OauthKnpuProviderConfigTransfer $oauthKnpuProviderConfigTransfer): OAuth2ClientInterface
    {
        return $this->clientRegistry->getClient($oauthKnpuProviderConfigTransfer->getClientNameOrFail());
    }

    protected function mapResourceOwnerToResourceOwnerTransfer(
        ResourceOwnerInterface $resourceOwner,
        OauthKnpuProviderConfigTransfer $oauthKnpuProviderConfigTransfer,
    ): ResourceOwnerTransfer {
        return (new ResourceOwnerTransfer())
            ->setId((string)$resourceOwner->getId())
            ->setProvider($oauthKnpuProviderConfigTransfer->getClientNameOrFail())
            ->setEmail($this->extractEmail($resourceOwner, $oauthKnpuProviderConfigTransfer));
    }

    protected function extractEmail(ResourceOwnerInterface $resourceOwner, OauthKnpuProviderConfigTransfer $oauthKnpuProviderConfigTransfer): ?string
    {
        ...
        if ($email === null) {
            $this->getLogger()->error(
                sprintf('OAuth provider returned no recognisable email claim for client "%s". Available claim keys: [%s]', $oauthKnpuProviderConfigTransfer->getClientNameOrFail(), implode(', ', array_keys($data))),
            );
        }
