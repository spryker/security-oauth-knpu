 ...
 */
class OauthMerchantUserInitiateController extends AbstractController
{
    ...
    public function indexAction(Request $request): RedirectResponse
    {
        ...
        $redirectResponse = $this->getFactory()
            ->createMerchantUserInitiator()
            ->getOauthRedirectResponse($clientName);

        if ($redirectResponse === null) {
            $this->addErrorMessage(static::MESSAGE_PROVIDER_NOT_FOUND);

            return $this->redirectResponse($this->getFactory()->getConfig()->getUrlMerchantPortalLogin());
        }
