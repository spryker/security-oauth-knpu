 ...
 */
class KnpuOauthMerchantUserClientStrategyPlugin extends AbstractPlugin implements OauthMerchantUserClientStrategyPluginInterface
{
    /**
     * {@inheritDoc}
     * - Returns true if the request targets the Knpu OAuth provider for merchant portal users.
     *
     * @api
     */
    public function isApplicable(ResourceOwnerRequestTransfer $resourceOwnerRequestTransfer): bool
    {
        return $this->getBusinessFactory()->createOauthMerchantPortalResourceOwnerReader()->isApplicable($resourceOwnerRequestTransfer);
    }
    ...
    {
        return $this->getBusinessFactory()->createOauthMerchantPortalResourceOwnerReader()->getResourceOwner($resourceOwnerRequestTransfer);
    }
}
